...
for name in names:
    id_list_rider = [i for i in range(len(all_rides)) if all_rides[i]['rider'] == name]
...
for group in groups:
    ...
    for i in range(1, len(group)):
        shared_segments = intersection(shared_segments, [seg['name'] for seg in all_rides[group[i]]['segment_efforts']])
    ...
    for segment_name in shared_segments:
        for idx_in_group, ride_idx in enumerate(group):
            activity_stats = all_rides[ride_idx]
